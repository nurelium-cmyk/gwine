...
export function Navigation() {
  ...
  const handleNavClick = (link: any) => {
    ...
    if (link.name === 'Hakkımızda') {
      scrollToSection('#museum');
      return;
    }
  ...
  return (
    <>
        ...
        <div className="hidden lg:flex items-center gap-8" role="menubar">
          {navLinks.map((link) => {
            return (
              ...
              >
                <button
                  onClick={() => !link.dropdown && handleNavClick(link)}
                    ...
                    <div className="bg-wine-800/95 backdrop-blur-md rounded-md overflow-hidden min-w-[180px] border border-white/10">
                      {link.dropdown.map((item) => (
                        <button
                          key={item.name}
                          onClick={() => {
                            const productId = productNameToId[item.name];
                            if (productId) {
      ...
      {/* Mobile Menu (moved outside nav so backdrop-blur affects page) */}
      <div
                  ...
                  <div>
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-500 ${
                        activeDropdown === link.name ? 'max-h-90' : 'max-h-0'
                      }`}
                      role="menu"
                    >
                      {link.dropdown.map((item) => (
